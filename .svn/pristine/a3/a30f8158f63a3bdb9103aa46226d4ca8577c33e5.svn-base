...
class Phoenix_Media_Rename {
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0, $title_from_post = 0, $name_from_post = 0, $check_post_parent = true, $force_lowercase = false, $force_serializiation = false, $rename = true) {
		...
		//add the new filename to file_parts structure
		$file_parts['new_filename'] = $new_filename;
		...
		$new_filename = self::clear_filename($options, $new_filename, $file_edited, $file_filename_ends_with, $file_extension, $file_subfolder, $post_parent_category);
		...
		$searches = self::get_attachment_urls($attachment_id, Operation::search, $file_parts, $new_filename);
		...
		//rename file
		$error_message = self::rename_files($options, $file_abs_path, $new_file_abs_path, $new_filename, $post->ID, $file_parts);
		...
		$metas = self::update_metadata(wp_get_attachment_metadata($attachment_id), wp_generate_attachment_metadata($attachment_id, $new_file_abs_path), $new_filename, $file_old_filename, $attachment_id, $file_path, $file_parts);
		...
		$replaces = self::get_attachment_urls($attachment_id, Operation::replace, $file_parts, $new_filename);
	 ...
	 */
	private static function rename_files($options, $file_abs_path, $new_file_abs_path, $new_filename, $post_id, $file_parts){
		$result = '';

		// Change attachment post metas & rename files
		if ($options->option_debug_mode){
			// //execute rename showing errors (if present)
			$result = self::rename_file_debug($file_abs_path, $new_file_abs_path, $new_filename, $post_id);

			if (($result == '') && ($file_parts['originalfilename'])){
				//if original_image exists, rename also the original file
				$result = self::rename_file_debug($file_parts['filepath'] . $file_parts['originalfilename'], $file_parts['filepath'] . $file_parts['new_filename'] . '.' . $file_parts['extension'], $file_parts['new_filename'], $post_id);
			}
		} else {
			//execute rename hiding errors (if present)
			$result = self::rename_file($file_abs_path, $new_file_abs_path, $new_filename, $post_id);
			if (($result == '') && ($file_parts['originalfilename'])){
				//if original_image exists, rename also the original file
				$result = self::rename_file($file_abs_path, $new_file_abs_path, $new_filename, $post_id);
				$result = self::rename_file($file_parts['filepath'] . $file_parts['originalfilename'], $file_parts['filepath'] . $file_parts['new_filename'] . '.' . $file_parts['extension'], $file_parts['new_filename'], $post_id);
			}
		}
	 ...
	 * @return string
	 */
	private static function clear_filename($options, $new_filename, $file_edited, $file_filename_ends_with, $file_extension, $file_subfolder, $post_parent_category){
		$result = $new_filename;
		...
		&& ! (self::ends_with($result, $filename_trailer))
		&& ! (self::ends_with($result, sanitize_file_name($filename_trailer)))){
	 ...
	 * @return array
	 */
	static function update_metadata($old_meta, $new_meta, $new_filename, $old_filename, $attachment_id, $file_path, $file_parts){
		$result = $old_meta;
		...
		if (array_key_exists('original_image', $result)){
			$result['original_image'] = $file_parts['new_filename'] . '.' . $file_parts['extension'];
			}
...
#region support functions
	...
	static function get_attachment_urls($attachment_id, $operation, $file_parts, $new_filename) {
		...
		if (wp_attachment_is_image($attachment_id)) {
			...
			if ($file_parts['originalfilename']){
				//if original_image exists, rename also the original file
				if (($operation == Operation::search)) {
					//if operation is replace, replace original filename with the new one
					$urls[] = $file_parts['baseurl'] . '/' . $file_parts['originalfilename'];
				} else {
			...
			foreach (get_intermediate_image_sizes() as $size) {
				...
				// if (($operation == Operation::replace) && ($file_parts['edited'])) {
				// 	// get filename
				...
				// 		//add original filename to array
				// 		$urls[] = $filepath . $file_parts[1] . '.' . $file_parts[4];
				// 	}
				// }
	...
	static private function generate_full_filename($file_parts, $new_filename){
		if ($file_parts['edited']){
			//if file has been scaled, remove the -scaled suffix
			$new_filename = substr($new_filename, 0, strlen('-scaled') * -1);
		}
		$result = $file_parts['baseurl'] . '/' . $new_filename . '.' . $file_parts['extension'];
