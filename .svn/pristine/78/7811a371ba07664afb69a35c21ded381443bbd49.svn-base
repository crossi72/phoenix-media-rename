...
class Phoenix_Media_Rename {
	 ...
	 * @return void
	 */
	static function pmr_get_option_boolean($options, $name ){
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0) {
		...
		$option_update_revisions = self::pmr_get_option_boolean( $options, 'pmr_update_revisions' );
		$option_remove_accents = self::pmr_get_option_boolean( $options, 'pmr_remove_accents' );
