...
class Phoenix_Media_Rename {
	...
	private static function rename_files($options, $file_abs_path, $new_file_abs_path, $new_filename, $post_id, $file_parts){
		$result = '';
		$filepath = $file_parts['filepath'];
		$file_extension = $file_parts['extension'];
		$original_filename = $file_parts['originalfilename'];
		$file_endswith = $file_parts['endswith'];
		...
		if ($options->option_debug_mode){
			...
			if (($result == '') && ($original_filename)){
				...
				$result = self::rename_file_debug($filepath . $original_filename . $file_endswith . '.' . $file_extension, $filepath . $file_parts['new_filename'] . $file_endswith . '.' . $file_extension, $file_parts['new_filename'], $post_id);
			}
		} else {
			...
			if (($result == '') && ($original_filename)){
				//if original_image exists, rename also the original file
				$result = self::rename_file($filepath . $original_filename . $file_endswith . '.' . $file_extension, $filepath . $file_parts['new_filename'] . $file_endswith . '.' . $file_extension, $file_parts['new_filename'], $post_id);
			}
		}
		...
		if (wp_attachment_is_image($attachment_id)) {
			...
			if ($file_parts['originalfilename']){
				...
				if (($operation == Operation::search)) {
					...
					$urls[] = $file_parts['baseurl'] . '/' . $file_parts['originalfilename'] . '.' . $file_parts['extension'];
				} else {
