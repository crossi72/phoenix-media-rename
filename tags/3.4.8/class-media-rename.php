...
class Phoenix_Media_Rename {
	 ...
	 * @return void
	 */
	static function get_option_text($options, $name, $default = ''){
		if ( isset($options[$name]) ){
			if ( $options[$name] ) {
				return $options[$name];
			}else{
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0, $title_from_post = 0, $name_from_post = 0, $check_post_parent = true ) {
		...
		$option_filename_header = self::get_option_text($options, 'pmr_filename_header', '');
		$option_filename_trailer = self::get_option_text($options, 'pmr_filename_trailer', '');

		//clear header and trailer
		$option_filename_header = trim($option_filename_header, " -");
		$option_filename_trailer = trim($option_filename_trailer, " -");
		...
		//add header to filename only if it not already present
		if (($option_filename_header != "") && (self::starts_with($new_filename, $option_filename_header) == -1)){
			$new_filename = $option_filename_header . ' ' . $new_filename;
		} else{
			//no header entered by user
		}

		//add trailer to filename only if it not already present
		if (($option_filename_trailer != "") && ! (self::ends_with($new_filename, $option_filename_trailer))){
			$new_filename = $new_filename . ' ' . $option_filename_trailer;
		} else{
	...
	static function update_metadata($old_meta, $new_meta, $new_filename, $old_filename){
		$result = $old_meta;
		define(shortpixelKey , 'thumbsOptList');
		...
		if (self::is_plugin_active(constant("pluginShortpixelImageOptimiser"))) {
			...
			if (array_key_exists(shortpixelKey, $result['ShortPixel'])){
				for ( $i = 0; $i < count( $result['ShortPixel'][shortpixelKey] ); $i++ ){
					$shortpixel_meta = $result['ShortPixel'][shortpixelKey][$i];

					$new_shortpixel_meta = str_replace( $old_filename, $new_filename, $shortpixel_meta );

					if ( $shortpixel_meta != $new_shortpixel_meta ){
						$result['ShortPixel'][shortpixelKey][$i] = $new_shortpixel_meta;
					}
				}
			}
		}
