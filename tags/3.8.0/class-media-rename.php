...
class Phoenix_Media_Rename {
	...
	function ajax_pnx_rename() {
		if (check_ajax_referer('phoenix_media_rename', '_wpnonce', 0)) {
			...
			if (! current_user_can('edit_post', $attachment_id)){
				...
				echo $this->do_rename($attachment_id, $new_filename, $retitle, $title_from_post, $name_from_post, false, $force_lowercase, $force_serializiation, $rename);
			}
	...
	private function rename_required(){
		...
		if ($_REQUEST['type'] == constant("actionRetitle")) {
	...
	private function retitle_required(){
		...
		//check if retitle is needed
		if ($_REQUEST['type'] == constant("actionRenameRetitle")) {
			//enable retitling if needed
			$result = true;
		}
