...
class Phoenix_Media_Rename {
	 ...
	 * @return void
	 */
	static function serialize_if_file_exists($filename, $extension){
		while (file_exists(wp_upload_dir()['basedir'] . '/' . $filename . '.' . $extension)){
	...
	static function file_exists_without_extension($full_filename){
		$files = glob(file_exists(wp_upload_dir()['basedir'] . '/' . $filename . '.*'));
			...
			if (!current_user_can('edit_post', $attachment_id)){
				...
				if ($name_from_post){
					...
					if (! $this->check_post_parent($post)){
						...
						// 	// $new_filename = self::serialize_if_file_exists($new_filename, '', true);
						// } else{
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0, $title_from_post = 0, $name_from_post = 0, $check_post_parent = true, $force_lowercase = false, $force_serializiation = false) {
		...
		if ($options->serialize_if_filename_present){
			$new_filename = self::serialize_if_file_exists($new_filename, $file_extension);
		} else{
			//sanitization disabled by user
		}
		...
		if (self::is_plugin_active(constant("pluginRedirection"))) {
			...
			self::add_redirection($file_old_filename, $new_filename, $file_extension, $file_subfolder, $options->option_create_redirection);
		}
