...
class Phoenix_Media_Rename {
	...
	static function increment_filename($filename){
		...
		if ($matches){
			...
			$number = $matches[0];
			$filename_without_number = $string[0];
			$number++;
			...
			if (!current_user_can('edit_post', $attachment_id)){
				...
				if ($name_from_post){
					...
					if (! $this->check_post_parent($post)){
						...
						if ($pmr_options->option_convert_to_lowercase){
							//convert the filename to lowercase if the option is active
				...
				echo $this->do_rename($attachment_id, $new_filename, $retitle, $title_from_post, $name_from_post, false, $force_lowercase, $force_serializiation);
			}
	 ...
	 * @return void
	 */
	static function do_rename($attachment_id, $new_filename, $retitle = 0, $title_from_post = 0, $name_from_post = 0, $check_post_parent = true, $force_lowercase = false, $force_serializiation = false) {
		//Variables
		...
		if (($options->option_filename_header != "") && (self::starts_with($new_filename, $options->option_filename_header) == -1)){
			$new_filename = $options->option_filename_header . ' ' . $new_filename;
		} else{
			//no header entered by user
		}

		//add trailer to filename only if it is not already present
		if (($options->option_filename_trailer != "") && ! (self::ends_with($new_filename, $options->option_filename_trailer))){
			$new_filename = $new_filename . ' ' . $options->option_filename_trailer;
		} else{
		...
		if ($options->serialize_if_filename_present){
			$new_filename = self::serialize_if_file_exists($new_filename, $file_extension, $file_subfolder);
		} else{
		...
		if (self::is_plugin_active(constant("pluginRedirection"))) {
			...
			self::add_redirection($file_old_filename, $new_filename, $file_extension, $file_subfolder, $options->option_create_redirection, constant("pluginRedirection"));
		}

		if (self::is_plugin_active(constant("pluginRankMath"))) {
			//plugin is active
			//Adding Redirection from old ORL to the new one
			self::add_redirection($file_old_filename, $new_filename, $file_extension, $file_subfolder, $options->option_create_redirection, constant("pluginRankMath"));
		}
	...
	static function update_metadata($old_meta, $new_meta, $new_filename, $old_filename){
		...
		if (self::is_plugin_active(constant("pluginShortpixelImageOptimiser"))) {
			...
			$result = self::update_shortpixel_metadata($result, $shortpixelKey, $old_filename, $new_filename);
			$shortpixelKey = 'excludeSizes';
			$result = self::update_shortpixel_metadata($result, $shortpixelKey, $old_filename, $new_filename);
		}

		//replace original filename (needed to ensure correct wp-cli management of thumnails renegeration)
