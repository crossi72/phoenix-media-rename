		...
		if (check_ajax_referer('phoenix_media_rename', '_wpnonce', 0)) {
			...
			if ($bulk_rename_in_progress){
				...
				if ($matches){
					...
					$zeroes = pmr_starts_with( $current_image_index, '0');
			...
			echo $this->do_rename($_REQUEST['post_id'], $new_filename, $retitle);
		}
		exit;
	}
 ...
 */
function pmr_starts_with($haystack, $needle) {
	$re = '/^[' . $needle . ']+/';
