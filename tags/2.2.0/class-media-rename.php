...
class Phoenix_Media_Rename {
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0) {
		...
		if ( ( $file_edited ) && ( $file_filename_ends_with == '-scaled' ) ){
		...
		// $file_abs_path = self::get_filename($post->ID);
		$file_abs_path = $file_path . $file_old_filename . '.' .$file_extension;
		$file_abs_dir = $file_path;
		// $new_file_abs_path = preg_replace('~[^/]+$~', $new_filename . $filename_ends_with . '.' . $file_parts['extension'], $file_abs_path);

		// $file_rel_path = get_post_meta($post->ID, '_wp_attached_file', 1);
		$file_rel_path = $file_old_filename . '.' .$file_extension;
		...
		//TODO:update original and -scaled versions
