...
class Phoenix_Media_Rename {
	...
	function print_js() {
		if ($this->is_media_rename_page) {
			wp_enqueue_script('phoenix-media-rename', plugins_url('js/scripts.min.js', __FILE__), array('jquery'), '3.1.0');
			?>
			...
			<script type="text/javascript">
				MRSettings = {
					'labels': {
						...
						'<?php echo constant("actionRenameFromPostTitle") ?>': '<?php echo __('Rename from Post', 'phoenix-media-rename') ?>',
						'<?php echo constant("actionRenameRetitleFromPostTitle") ?>': '<?php echo __('Rename & Retitle from Post', 'phoenix-media-rename') ?>'
					}
				};
			</script>
	...
	static function check_post_parent( $post ){
		...
		if ( ! $post_parent ){
			//no post found
			echo __('The media is not attached to a post!', 'phoenix-media-rename');
			return false;
		}
		...
		if ( ! $new_filename ){
			//no title set
			echo __('The post has no title!', 'phoenix-media-rename');
			return false;
		}
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0, $title_from_post = 0, $name_from_post = 0, $check_post_parent = true ) {
		...
		$option_debug_mode = self::get_option_boolean( $options, 'pmr_debug_mode' );
		...
		try{
			if ( self::is_plugin_active( 'archivarix-external-images-importer/archivarix-external-images-importer.php' ) ) {
				//plugin is activated, remove last . added by archivarix
		...
		if ( self::is_plugin_active( 'amazon-s3-and-cloudfront/wordpress-s3.php' ) ) {
			//plugin is activated
		...
		if ( !is_writable( realpath( $file_abs_dir ) ) ) return __('The media containing directory is not writable!', 'phoenix-media-rename');
		...
		if ( self::is_plugin_active( 'smart-slider-3/smart-slider-3.php' ) ) {
			//plugin is activated
	...
	static function update_metadata($old_meta, $new_meta, $new_filename, $old_filename){
		...
		if ( self::is_plugin_active( 'shortpixel-image-optimiser/wp-shortpixel.php' ) ) {
