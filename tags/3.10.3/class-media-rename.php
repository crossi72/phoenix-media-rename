...
class Phoenix_Media_Rename {
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0, $title_from_post = 0, $name_from_post = 0, $check_post_parent = true, $force_lowercase = false, $force_serializiation = false, $rename = true) {
		...
		if ((! $rename) && ($retitle)){
			...
			//Validations
			$validation_message = self::validate($post, $attachment_id, $options, $file_info);
			...
			if ($validation_message != ''){
				...
				$metas = self::update_metadata(wp_get_attachment_metadata($attachment_id), wp_generate_attachment_metadata($attachment_id, $file_info->new_file_abs_path), $attachment_id, $file_info->new_filename, $file_info->file_old_filename, $file_info->file_path, $file_info);
	 ...
	 * @return void
	 */
	private static function validate($post, $attachment_id, $options, $file_info){
	...
	static function update_metadata($old_meta, $new_meta, $attachment_id, $file_info){//, $new_filename, $old_filename, $file_path, $file_parts){
		...
		$result = pmr_plugins::update_shortpixel_metadata($result, $file_info->old_filename, $file_info->new_filename, $attachment_id, $file_info->file_path);
		...
		if (array_key_exists('original_image', $result)){
			$result['original_image'] = $file_info->new_file_name . '.' . $file_info->extension;
			}
