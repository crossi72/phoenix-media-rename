...
class Phoenix_Media_Rename {
	...
	function print_js() {
		if ($this->is_media_rename_page) {
			...
			<script type="text/javascript">
				MRSettings = {
					'labels': {
						'rename': '<?php echo  __('Rename', 'phoenix-media-rename') ?>',
						'rename_retitle': '<?php echo  __('Rename & Retitle', 'phoenix-media-rename') ?>'
					}
				};
			</script>
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0) {
		...
		$new_filename = sanitize_title($new_filename);
		...
		if ( ($new_filename != sanitize_file_name($new_filename)) || preg_match('~[^\p{Common}\p{Latin}]~u', $new_filename) ) return __('Bad characters or invalid filename!', 'phoenix-media-rename');
