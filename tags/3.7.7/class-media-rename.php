...
class Phoenix_Media_Rename {
		...
		while ($posts = get_posts(array('post_type' => $post_types, 'post_status' => 'any', 'numberposts' => 100, 'offset' => $i * 100))) {
			foreach ($posts as $post) {
				...
				foreach ($metas as $key => $meta) {
					switch ($key){
						case '_elementor_css':
							pmr_plugins::update_elementor_data(elementor_element::css, $post->ID);
						break;
						case '_elementor_data':
							pmr_plugins::update_elementor_data(elementor_element::data, $post->ID, $key, $searches, $replaces);
						break;
						default:
							//update wp_postmeta
	...
	static function update_metadata($old_meta, $new_meta, $new_filename, $old_filename){
		...
		if (pmr_plugins::is_plugin_active(constant("pluginShortpixelImageOptimiser"))) {
			...
			$result = pmr_plugins::update_shortpixel_metadata($result, $shortpixelKey, $old_filename, $new_filename);
